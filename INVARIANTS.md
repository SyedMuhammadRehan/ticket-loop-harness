...
| 62 | The codebase map names the command and the commit it was built from | `survey.js` → `writeMap` | `survey.test.js` :: `the map is stamped with the source command and the HEAD it was read from` | A map with no provenance cannot be judged stale, and a configured source is a command the profile trusts by name |
